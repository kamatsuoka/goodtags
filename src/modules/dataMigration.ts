 ...
 */
export const restoreFromBackup = async (backupContent: string): Promise<boolean> => {
  try {
    ...
    return new Promise((resolve) => {
      Alert.alert(
        "Restore Backup",
        `This will restore data from ${new Date(backupData.timestamp).toLocaleDateString()}. Your current data will be replaced. Continue?`,
        [
          {
            text: "Cancel",
            style: "cancel",
          ...
          },
          {
            ...
            onPress: async () => {
              try {
                ...
                await AsyncStorage.setItem("persist:root", JSON.stringify(backupData.reduxState))
                ...
                Alert.alert("Restore Failed", "There was an error restoring your data. Please try again.")
                resolve(false)
              }
            }
          }
